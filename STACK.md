...
- **Fetch window:** one poll fetches at most a 7-day hourly window (`ROLLING_WINDOW_HOURS`); do not widen the default rolling window without a documented reason. Upstream calls have a typed failure and an explicit fail-quiet (timer) vs raise (setup) decision — never a silent empty.
